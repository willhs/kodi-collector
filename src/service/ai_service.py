...
def classify_media_file(raw_file_name) -> Media:
    gpt_system_prompt = """
...
Please classify the file given the filename as either a tv show ("tv show") or movie ("movie") based on your knowledge of media.
    ...
    print(f"Classified as {gpt_response}")
...
def rename_movie_filename(name) -> string:
    gpt_system_prompt = """
You are a smart filename parser for Kodi media player, specializing in movie files. Your task is to clean up raw filenames into a preferred format: "Movie Name (Year)". Rely on your extensive knowledge base to deduce the year of release. If multiple movies exist with the same name, make your best educated guess to select the most probable one. If you cannot determine the year, it's acceptable to leave it out. Here are good examples:
